...
|---|---|---|
| **Inj. Atropine** | **500 Amps** | **Hospurush** |
| **Inj. Adrenaline** | **500 Amps** | **Hospurush** |
...
2. **Vendor Contact** — for Inj. Atropine and Inj. Adrenaline, the order is dispatched directly to **Hospurush**.
...
- **Purchase Officer** — responsible for maintaining vendor relationships (Hospurush, etc.) and ensuring timely payments to prevent supply holds.
...
- **Trigger rule:** when stock of `adrenaline` OR `atropine` falls **below 500 units**, the agent (a) posts an alert to Slack `#pharmacy-alerts` for human acknowledgement, and (b) generates a draft PO addressed to **Hospurush**.
